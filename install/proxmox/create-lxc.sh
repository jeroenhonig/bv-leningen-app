...
TEMPLATE_FILE=$(pveam available | grep -o 'debian-12[^ ]*\.tar\.\(gz\|xz\|zst\)' | sort -V | tail -n1)
...
pct exec $CTID -- bash -c '
  ...
  curl -s https://raw.githubusercontent.com/jeroenhonig/bv-leningen-app/main/install/lxc/setup.sh > /root/setup.sh && \
